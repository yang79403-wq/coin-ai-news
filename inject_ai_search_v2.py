...
<section id="ai-search" class="section coin-ai-search" aria-label="币智通AI智能搜索">
  <h2>🤖 币智通 AI</h2>
  <span class="ai-tag">智能理解问题：钱币、版别、行情、收藏知识都可以直接问</span>
  <div class="coin-ai-chat">
    ...
    <div id="coin-ai-status" class="coin-ai-status">🟡 智能接口待连接：当前使用网站资料检索；接入安全 AI 服务后自动升级为多轮智能回答。</div>
  </div>
...
(function(){
  ...
  function localAnswer(q){
    ...
    return hits.length?'我先从币智通当前网页资料中找到这些相关内容：\n\n• '+hits.join('\n• ')+'\n\n如果你需要具体价格，我建议再提供：钱币名称、版别、品相和正反面图片。这样接入智能行情检索后，我可以进一步综合成交数据分析。':'当前网页资料中暂时没有直接匹配内容。你可以换成具体钱币名称、版别或上传图片继续问。';
  }
  function send(q,imageData){
    ...
    if(!endpoint){setTimeout(function(){var a=localAnswer(q||'钱币图片');add('assistant',a);history.push({role:'assistant',content:a});status.textContent='🟡 当前为网站资料模式。配置安全 AI 服务后启用多轮智能回答与图片理解。';},250);return;}
