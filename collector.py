...
UA='HongshengCollectionBot/6.1 (+https://yang79403-wq.github.io/coin-ai-news/)'
...
def category(text):
    t=text.lower()
    rules=[('纸币',['纸币','人民币','冠号','纪念钞']),('银元',['银元','袁大头','孙小头','船洋','龙洋','大清银币','光绪元宝']),('古钱币',['古钱','通宝','重宝','元宝','咸丰','乾隆','顺治','康熙']),('铜元',['铜元','铜币','大清铜币','机制铜']),('金银币',['金币','金银币','熊猫币']),('纪念币',['纪念币']),('邮票',['邮票','邮品']),('硬币',['硬币','分币','长城币'])]
    for c,ks in rules:
...
def parse_pm001(base):
    ...
    for board_url,board_title in boards[:12]:
        try: bs=BeautifulSoup(fetch(board_url).text,'html.parser')
        except: continue
        links=[]
        for a in bs.find_all('a',href=True):
            h=a['href']; title=clean(a.get_text(' ',strip=True))
            if title and 'dispbbs.asp' in h.lower(): links.append((urljoin(board_url,h),title))
        for topic,title in links[:35]:
            try:
                text=clean(BeautifulSoup(fetch(topic).text,'html.parser').get_text(' ',strip=True))
                ...
                records.append({'category':category(title+' '+text[:1400]),'name':title[:120],'status':status,'price':f'{candidates[0]:g}元','date':time.strftime('%Y-%m-%d'),'source':'一尘网','source_url':topic,'board':board_title[:80]})
...
def parse_yy11(base):
    ...
    for a in soup.find_all('a',href=True):
        h=a['href']; title=clean(a.get_text(' ',strip=True))
        if title and '/c2c/topic/' in h: links.append((urljoin(base,h),title))
    for topic,title in links[:50]:
        try:
            text=clean(BeautifulSoup(fetch(topic).text,'html.parser').get_text(' ',strip=True))
            m=re.search(r'物品成交价[^0-9]{0,60}(?:¥|￥)?\s*([0-9][0-9,]*(?:\.[0-9]+)?)\s*元?',text)
            if m and '成交于' in text and '流拍' not in text:
                records.append({'category':category(title+' '+text[:1400]),'name':title[:120],'status':'成交','price':f"{float(m.group(1).replace(',','')):g}元",'date':time.strftime('%Y-%m-%d'),'source':'钱币天堂','source_url':topic})
        except Exception: pass
        time.sleep(.05)
    return records,health
...
def fujian():
    ...
    for name,cat,period,source,url in seeds:
        ...
        try:
            soup=BeautifulSoup(fetch(url).text,'html.parser'); text=clean(soup.get_text(' ',strip=True)); rec['features']=text[:900]
...
def main():
    ...
    print('market=',len(market),'fujian=',len(json.loads((DATA/'fujian.json').read_text())['records']))
